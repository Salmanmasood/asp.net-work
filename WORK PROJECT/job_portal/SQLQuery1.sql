...
select * from tblImages_login_info
...
@inimage_login_id_fk int
)
as
Begin
    Insert into tblImages_login_info(Name,Size,ImageData,inimage_login_id_fk)
    values (@Name, @Size, @ImageData,@inimage_login_id_fk)
...
Select ImageData from tblImages_login_info where Id=@Id AND inimage_login_id_fk=@inimage_login_id_fk

End


Select ImageData from tblImages_login_info where Id=15 AND inimage_login_id_fk=1

delete from tblImages_login_info

select max(id) from tblImages_login_info where inimage_login_id_fk=1
